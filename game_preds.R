...
make_game_table <- function(df, title_str, subtitle_str, group_stage = T) {
  right_border_cols <- if(group_stage) {
    c("lambda_2")
  } else {
    c("lambda_2", "loss")
  }
  df %>%
    ...
    tab_style(style = list(cell_borders(sides = "right", color = "black", weight = px(3))),
              locations = list(cells_body(columns = right_border_cols))) %>%
    ...
    { if(group_stage) {
        ...
        tab_spanner(tab_spanner(., label = 'Regulation Result', columns = c('win', 'draw', 'loss')),
                    label = 'To Advance', columns = c('win_ko', 'loss_ko'))
      } } %>%
    ...
    { if(group_stage) {
        ...
        cols_label(., date = 'Date', team1 = 'Team 1', logo1 = '', team2 = 'Team 2', logo2 = '',
                   ...
                   win = 'Team 1 Win', draw = 'Extra Time', loss = 'Team 2 Win',
...
make_mw_table <- function(df, subtitle_str) {
  ...
  bind_cols(df1, df2) %>%
    ...
    data_color(columns = c(lambda_1_1, lambda_2_1, lambda_1_2, lambda_2_2),
               fn = scales::col_numeric(palette = ggsci::rgb_material('amber', n = 100),
                                        domain = range(preds[, c('lambda_1', 'lambda_2')], na.rm = T))) %>%
    data_color(columns = c(win_1, draw_1, loss_1, win_2, draw_2, loss_2),
               fn = scales::col_numeric(palette = ggsci::rgb_material('amber', n = 100), domain = c(0, 1))) %>%
    tab_style(style = list(cell_borders(sides = "bottom", color = "black", weight = px(3))),
              locations = list(cells_column_labels(columns = gt::everything()))) %>%
    tab_style(style = list(cell_borders(sides = "bottom", color = "black", weight = px(3))),
              locations = list(cells_body(rows = group_borders))) %>%
    tab_style(style = list(cell_borders(sides = "right", color = "black", weight = px(3))),
              locations = list(cells_body(columns = c(group_1, lambda_2_1, loss_1, group_2, lambda_2_2)))) %>%
    ...
    text_transform(locations = cells_body(columns = c(logo1_1, logo1_2, logo2_1, logo2_2)),
                   fn = function(x) map_chr(x, ~local_image(filename = as.character(.x), height = 30))) %>%
    cols_label(date_1 = 'Date', team1_1 = 'Team 1', logo1_1 = '', team2_1 = 'Team 2', logo2_1 = '',
    ...
    tab_source_note("Luke Benz (@recspecs730)") %>%
    tab_source_note("Data: github.com/martj42/international_results | Country Images: Flaticon.com") %>%
    tab_header(title = html(paste0("<img src='", file.path(getwd(), 'flags/fifa_logo.jpg'), "' height='160'>")),
               subtitle = md(paste0('**', subtitle_str, '**'))) %>%
    tab_options(column_labels.font.size = 20,
                heading.subtitle.font.size = 30,
                heading.subtitle.font.weight = 'bold',
...
mw1 <-
  df_preds %>%
  filter(date >= as.Date('2026-06-11'), date <= as.Date('2026-06-17'))
mw2 <-
  df_preds %>%
  filter(date >= as.Date('2026-06-18'), date <= as.Date('2026-06-23'))
mw3 <-
  df_preds %>%
  filter(date >= as.Date('2026-06-24'), date <= as.Date('2026-06-27'))
